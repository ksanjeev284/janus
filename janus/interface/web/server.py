...
async def get_report_html():
    """Generate HTML report from latest scan results."""
    from datetime import datetime
    
    if not scan_results and not autoscan_reports:
        return HTMLResponse("""
        <html><body style="background:#0d1117;color:#fff;font-family:sans-serif;padding:40px;text-align:center;">
        <h1>🔱 No Scan Results</h1>
        <p>Run a scan first to generate a report.</p>
        </body></html>
        """)
    
    # Build report from all available results
    html = f'''
<!DOCTYPE html>
<html>
<head>
    <title>Janus Security Report</title>
    <style>
        ...
        .stat {{ display: inline-block; background: #21262d; padding: 10px 20px; border-radius: 8px; margin: 5px; text-align: center; }}
...
'''
    ...
    # Add BOLA scan results if available
    if scan_results:
        html += '<div class="card"><h2>BOLA/IDOR Scans</h2>'
        for scan_id, report in scan_results.items():
            report_dict = report.to_dict()
            vulns = len(report_dict.get('vulnerabilities', []))
            html += f'''
            <div style="margin:10px 0;padding:10px;background:#0d1117;border-radius:8px;">
                <strong>Scan: {scan_id}</strong> - {vulns} vulnerabilities found
            </div>
            '''
        html += '</div>'
    ...
    <footer style="text-align:center;margin-top:40px;color:#8b949e;">
        Generated by Janus Security Scanner
    </footer>
</body>
</html>
'''
